...
function visibleSalesOrder(db, order, user) {
  ...
  const base = {
    ...
    customerPhone: customer?.phone || order.customerPhone || "",
    customerCountry: order.destinationCountry || customer?.country || "",
    customerAddress: [order.destinationCountry, order.destinationAddress].filter(Boolean).join(" ") || customer?.address || "",
